...
## read in data and only select genes with > 2 tpm ##
...
TF_family <- read.csv("/nbi/Research-Groups/NBI/Cristobal-Uauy/PB_AFLF/control_timecourse/TF_analysis/triticum_aestivum_TFs_final_high_conf.csv", header =T)
head(TF_family)
...
# merge together counts  and tpmData_FLB_av
...
print("dimensions of count_matrix TFs in G >1 tpm:")
dim(count_matrix_G)
...
setwd("/nbi/Research-Groups/NBI/Cristobal-Uauy/PB_AFLF/control_timecourse/TGAC_kallisto_analysis/kallisto_results_bootstrap/results/9_EBSeqHMM_TF_only_new_annotation/")
