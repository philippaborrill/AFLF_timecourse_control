...
out_dir <- "Y:/PB_AFLF/control_timecourse/TGAC_kallisto_analysis/kallisto_results_bootstrap/results/1_SOM_analysis/"
...
plot(hclust(dist(av_tpm_sc.som$codes)))
tiff(filename = paste0(out_dir, "FLB_SOM_hclust_all_genes.tiff", sep = ""), width = 1000, height = 500)
plot(hclust(dist(av_tpm_sc.som$codes)))
dev.off()
...
som.hc <- cutree(hclust(dist(av_tpm_sc.som$codes)), 11)
plot(av_tpm_sc.som, main = "FLB", codeRendering = "lines")
add.cluster.boundaries(av_tpm_sc.som, som.hc)

tiff(filename = paste0(out_dir, "FLB_SOM_11groups_all_transcript.tiff", sep = ""), width = 1000, height = 500)
som.hc <- cutree(hclust(dist(av_tpm_sc.som$codes)), 11)
plot(av_tpm_sc.som, main = "FLB", codeRendering = "lines")
add.cluster.boundaries(av_tpm_sc.som, som.hc)
dev.off()
...
# try adding background colour according to group - rainbow
tiff(filename = paste0(out_dir, "FLB_SOM_11groups_all_transcript_coloured_rainbow_no_outline.tiff", sep = ""), width = 1000, height = 500)
som.hc <- cutree(hclust(dist(av_tpm_sc.som$codes)), 11)
plot(av_tpm_sc.som, main = "FLB", codeRendering = "lines",  palette.name=palette(rainbow(11,s=0.5,alpha=1)), bgcol=som.hc, ncolors=11 )
#add.cluster.boundaries(av_tpm_sc.som, som.hc, lwd=3)
dev.off()

library(RColorBrewer)
# try adding background colour according to group - r color brewer
tiff(filename = paste0(out_dir, "FLB_SOM_11groups_all_transcript_coloured_rainbow_no_outline.tiff", sep = ""), width = 1000, height = 500)
som.hc <- cutree(hclust(dist(av_tpm_sc.som$codes)), 11)
plot(av_tpm_sc.som, main = "FLB", codeRendering = "lines",  palette.name=palette(brewer.pal(12,"Set3")), bgcol=som.hc, ncolors=11 )
#add.cluster.boundaries(av_tpm_sc.som, som.hc, lwd=3)
dev.off()


# try adding background colour according to group - r color brewer with legend
tiff(filename = paste0(out_dir, "FLB_SOM_11groups_all_transcript_coloured_brewer_palette_outline_legend.tiff", sep = ""), width = 1000, height = 500)
som.hc <- cutree(hclust(dist(av_tpm_sc.som$codes)), 11)
plot(av_tpm_sc.som, main = "FLB", codeRendering = "lines",  palette.name=palette(brewer.pal(12,"Set3")), bgcol=som.hc, ncolors=11 )
...
# try adding background colour according to group - r color brewer with legend
tiff(filename = paste0(out_dir, "FLB_SOM_11groups_all_transcript_coloured_brewer_palette_no_outline_legend.tiff", sep = ""), width = 1000, height = 500)
som.hc <- cutree(hclust(dist(av_tpm_sc.som$codes)), 11)
plot(av_tpm_sc.som, main = "FLB", codeRendering = "lines",  palette.name=palette(brewer.pal(12,"Set3")), bgcol=som.hc, ncolors=11 )
legend(-1.05,6, legend=unique(som.hc), fill=palette(brewer.pal(12,"Set3")), bty="n")
dev.off()
...
lengths <- read.csv(file="length_per_gene.csv", header=T)
head(lengths)
